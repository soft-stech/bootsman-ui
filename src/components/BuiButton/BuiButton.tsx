...
import { defineProps, withDefaults, computed } from 'vue-demi'
import { ButtonColor, ButtonSize } from './types.ts'

const IBuiButtonProps = {
  disabled: Boolean,
  color: {
...
export default {
  props: withDefaults(defineProps(IBuiButtonProps), {
    disabled: false,
    color: 'primary',
    size: 'md'
  }),
  ...
  render() {
    return (
      <button class={this.buttonClasses} disabled={this.disabled}>
        ...
        {this.$slots.default ? this.$slots.default() : 'Primary'}
